...
float4 main(
  ...
  _1054 = (PostProcess_000.PostProcessConstant_Z_000[17].y) + 2.0f;
  _1060 = (exp2(_1054 * log2(_1049)) + -1.0f) / (_1049 + -1.0f);
  _1072 = ((pow(_1050, _1054)) + -1.0f) / (_1050 + -1.0f);
  _1084 = ((pow(_1051, _1054)) + -1.0f) / (_1051 + -1.0f);
  _1091 = saturate(select((!(_1049 == 1.0f)), ((_1060 + -1.0f) / _1060), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _1054)));
  _1092 = saturate(select((!(_1050 == 1.0f)), ((_1072 + -1.0f) / _1072), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _1054)));
  _1093 = saturate(select((!(_1051 == 1.0f)), ((_1084 + -1.0f) / _1084), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _1054)));

  float3 output;
  if (RENODX_TONE_MAP_TYPE) {
    output = float3(_1091, _1092, _1093);
    output = renodx::draw::RenderIntermediatePass(output);
    SV_Target.rgb = output;
  } else {
    _1115 = select((_1091 <= 0.0031308000907301903f), (_1091 * 12.920000076293945f), (((pow(_1091, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    _1116 = select((_1092 <= 0.0031308000907301903f), (_1092 * 12.920000076293945f), (((pow(_1092, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    _1117 = select((_1093 <= 0.0031308000907301903f), (_1093 * 12.920000076293945f), (((pow(_1093, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));

    output = float3(_1115, _1116, _1117);

    _1138 = (int)(uint(SV_Position.x)) & 63;
    _1139 = (int)(uint(SV_Position.y)) & 63;
    _1143 = t6.Load(int4(_1138, _1139, _978, 0));
    SV_Target.x = (((((float4)(t1.Load(int4(_1138, _1139, _978, 0)))).x) * select((output.r <= 0.0f), 0.0f, exp2(floor(log2(output.r)) + -6.0f))) + output.r);
    SV_Target.y = ((_1143.x * select((output.g <= 0.0f), 0.0f, exp2(floor(log2(output.g)) + -6.0f))) + output.g);
    SV_Target.z = ((_1143.y * select((output.b <= 0.0f), 0.0f, exp2(floor(log2(output.b)) + -5.0f))) + _1117);
  }
  SV_Target.w = _380;
  return SV_Target;
}