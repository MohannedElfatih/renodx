...
float4 main(
  ...
  _708 = ((exp2(_686.x * 13.450128555297852f) + -1.0f) * 0.0029786902014166117f) * (User_000.UserConstant_Z_000[4].x);
  _709 = (PostProcess_000.PostProcessConstant_Z_000[17].y) + 2.0f;
  _715 = (exp2(_709 * log2(_708)) + -1.0f) / (_708 + -1.0f);
  _722 = ((exp2(_686.y * 13.450128555297852f) + -1.0f) * 0.0029786902014166117f) * (User_000.UserConstant_Z_000[4].y);
  _728 = ((pow(_722, _709)) + -1.0f) / (_722 + -1.0f);
  _735 = ((exp2(_686.z * 13.450128555297852f) + -1.0f) * 0.0029786902014166117f) * (User_000.UserConstant_Z_000[4].z);
  _741 = ((pow(_735, _709)) + -1.0f) / (_735 + -1.0f);
  _748 = saturate(select((!(_708 == 1.0f)), ((_715 + -1.0f) / _715), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _709)));
  _749 = saturate(select((!(_722 == 1.0f)), ((_728 + -1.0f) / _728), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _709)));
  _750 = saturate(select((!(_735 == 1.0f)), ((_741 + -1.0f) / _741), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _709)));

  float3 output;
  if (RENODX_TONE_MAP_TYPE) {
    output = float3(_748, _749, _750);
    output = renodx::draw::RenderIntermediatePass(output);
    SV_Target.rgb = output;
  } else {
    _772 = select((_748 <= 0.0031308000907301903f), (_748 * 12.920000076293945f), (((pow(_748, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    _773 = select((_749 <= 0.0031308000907301903f), (_749 * 12.920000076293945f), (((pow(_749, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    ...
    output = float3(_772, _773, _774);

    _795 = asint((Global_000.GlobalCB_Z_000.GlobalConstant_Z_000[1].w));
    _798 = (int)(uint(SV_Position.x)) & 63;
    _799 = (int)(uint(SV_Position.y)) & 63;
    _804 = t6.Load(int4(_798, _799, _795, 0));
    SV_Target.x = (((((float4)(t1.Load(int4(_798, _799, _795, 0)))).x) * select((output.r <= 0.0f), 0.0f, exp2(floor(log2(output.r)) + -6.0f))) + output.r);
    SV_Target.y = ((_804.x * select((output.g <= 0.0f), 0.0f, exp2(floor(log2(output.g)) + -6.0f))) + output.g);
    SV_Target.z = ((_804.y * select((output.b <= 0.0f), 0.0f, exp2(floor(log2(output.b)) + -5.0f))) + _774);
  }
  SV_Target.w = _134.w;
  return SV_Target;
}