...
float4 main(
  ...
  _664 = (PostProcess_000.PostProcessConstant_Z_000[17].y) + 2.0f;
  _670 = (exp2(_664 * log2(_659)) + -1.0f) / (_659 + -1.0f);
  _682 = ((pow(_660, _664)) + -1.0f) / (_660 + -1.0f);
  _694 = ((pow(_661, _664)) + -1.0f) / (_661 + -1.0f);
  _701 = saturate(select((!(_659 == 1.0f)), ((_670 + -1.0f) / _670), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _664)));
  _702 = saturate(select((!(_660 == 1.0f)), ((_682 + -1.0f) / _682), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _664)));
  _703 = saturate(select((!(_661 == 1.0f)), ((_694 + -1.0f) / _694), (((PostProcess_000.PostProcessConstant_Z_000[17].y) + 1.0f) / _664)));

  float3 output;
  if (RENODX_TONE_MAP_TYPE) {
    output = float3(_701, _702, _703);
    output = renodx::draw::RenderIntermediatePass(output);
    SV_Target.rgb = output;
  } else {
    _725 = select((_701 <= 0.0031308000907301903f), (_701 * 12.920000076293945f), (((pow(_701, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    _726 = select((_702 <= 0.0031308000907301903f), (_702 * 12.920000076293945f), (((pow(_702, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    _727 = select((_703 <= 0.0031308000907301903f), (_703 * 12.920000076293945f), (((pow(_703, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));

    output = float3(_725, _726, _727);

    _748 = (int)(uint(SV_Position.x)) & 63;
    _749 = (int)(uint(SV_Position.y)) & 63;
    _753 = t2.Load(int4(_748, _749, _588, 0));
    SV_Target.x = (((((float4)(t1.Load(int4(_748, _749, _588, 0)))).x) * select((output.r <= 0.0f), 0.0f, exp2(floor(log2(output.r)) + -6.0f))) + output.r);
    SV_Target.y = ((_753.x * select((output.g <= 0.0f), 0.0f, exp2(floor(log2(output.g)) + -6.0f))) + output.g);
    SV_Target.z = ((_753.y * select((output.b <= 0.0f), 0.0f, exp2(floor(log2(output.b)) + -5.0f))) + _727);
  }
  SV_Target.w = _372;
  return SV_Target;
}