...
float4 main(
  ...
  float _221 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(_25 + -0.5f)) * ((((_15 / _16) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _222 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(_26 + -0.5f))) * 5.0f);
  float _235 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_221, _222), float2(_221, _222)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_235 * ((_62 - (_194 * _62)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_235 * ((_63 - (_194 * _63)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_235 * ((_64 - (_194 * _64)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
