...
float4 main(
  ...
  float _284 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(O_Uv.x + -0.5f)) * ((((float((int)(frame_consts.vp_dim.x)) / float((int)(frame_consts.vp_dim.y))) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _285 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(O_Uv.y + -0.5f))) * 5.0f);
  float _298 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_284, _285), float2(_284, _285)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_298 * ((_125 - (_257 * _125)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_298 * ((_126 - (_257 * _126)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_298 * ((_127 - (_257 * _127)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
