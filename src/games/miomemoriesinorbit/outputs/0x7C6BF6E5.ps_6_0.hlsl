...
float4 main(
  ...
  float _223 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(O_Uv.x + -0.5f)) * ((((float((int)(frame_consts.vp_dim.x)) / float((int)(frame_consts.vp_dim.y))) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _224 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(O_Uv.y + -0.5f))) * 5.0f);
  float _237 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_223, _224), float2(_223, _224)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_237 * ((_125 - (_196 * _125)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_237 * ((_126 - (_196 * _126)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_237 * ((_127 - (_196 * _127)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
