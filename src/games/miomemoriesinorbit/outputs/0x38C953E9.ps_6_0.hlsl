...
float4 main(
  ...
  float _240 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(O_Uv.x + -0.5f)) * ((((float((int)(frame_consts.vp_dim.x)) / float((int)(frame_consts.vp_dim.y))) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _241 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(O_Uv.y + -0.5f))) * 5.0f);
  float _254 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_240, _241), float2(_240, _241)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_254 * ((_125 - (_213 * _125)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_254 * ((_126 - (_213 * _126)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_254 * ((_127 - (_213 * _127)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
