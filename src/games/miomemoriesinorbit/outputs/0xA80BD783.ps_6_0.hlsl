...
float4 main(
  ...
  float _210 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(O_Uv.x + -0.5f)) * ((((float((int)(frame_consts.vp_dim.x)) / float((int)(frame_consts.vp_dim.y))) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _211 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(O_Uv.y + -0.5f))) * 5.0f);
  float _224 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_210, _211), float2(_210, _211)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_224 * ((_51 - (_183 * _51)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_224 * ((_52 - (_183 * _52)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_224 * ((_53 - (_183 * _53)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
