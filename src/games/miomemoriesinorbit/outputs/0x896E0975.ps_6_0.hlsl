...
float4 main(
  ...
  float _154 = log2(max(5.0f, (-0.0f - (_145 - frame_consts.game_plane_blend.x)))) * 10.0f;
  float _155 = floor(_154);
  float _156 = _154 - _155;
  float _158 = exp2(_155 * 0.10000000149011612f);
  float _159 = 1.0f / _158;
  float _160 = min(_159, 0.20000000298023224f);
  float _162 = min((_159 * 0.9330329895019531f), 0.20000000298023224f);
  float _163 = frame_consts.game_plane_blend.x - _158;
  float _169 = frame_consts.game_plane_blend.x - (_158 * 1.0717734098434448f);
  float _179 = paper_tex.SampleLevel(sampler_bilinear_wrap, float2((((_163 * _146) + (frame_consts.paper_xf[3].x)) * _160), (((_163 * _147) + (frame_consts.paper_xf[3].y)) * _160)), 0.0f);
  float _182 = paper_tex.SampleLevel(sampler_bilinear_wrap, float2((((_169 * _146) + (frame_consts.paper_xf[3].x)) * _162), (((_169 * _147) + (frame_consts.paper_xf[3].y)) * _162)), 0.0f);
  float _189 = ((1.0f - (_179.x * (1.0f - _156))) - (_182.x * _156)) * frame_consts.paper_mult;
  float _216 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(_25 + -0.5f)) * ((((_15 / _16) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _217 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(_26 + -0.5f))) * 5.0f);
  float _230 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_216, _217), float2(_216, _217)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_230 * ((_101 - (_189 * _101)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_230 * ((_102 - (_189 * _102)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_230 * ((_103 - (_189 * _103)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
