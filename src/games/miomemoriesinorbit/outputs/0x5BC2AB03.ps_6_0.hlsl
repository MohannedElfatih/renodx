...
float4 main(
  ...
  float _174 = exp2(log2(saturate((frame_consts.params.vignette.intensity * abs(O_Uv.x + -0.5f)) * ((((float((int)(frame_consts.vp_dim.x)) / float((int)(frame_consts.vp_dim.y))) + -1.0f) * frame_consts.params.vignette.rounded) + 1.0f))) * 5.0f);
  float _175 = exp2(log2(saturate(frame_consts.params.vignette.intensity * abs(O_Uv.y + -0.5f))) * 5.0f);
  float _188 = 1.0f - ((1.0f - exp2(log2(saturate(1.0f - dot(float2(_174, _175), float2(_174, _175)))) * 6.0f)) * 0.8500000238418579f);
  SV_Target.x = ((_188 * ((_51 - (_147 * _51)) - frame_consts.params.vignette.color.x)) + frame_consts.params.vignette.color.x);
  SV_Target.y = ((_188 * ((_52 - (_147 * _52)) - frame_consts.params.vignette.color.y)) + frame_consts.params.vignette.color.y);
  SV_Target.z = ((_188 * ((_53 - (_147 * _53)) - frame_consts.params.vignette.color.z)) + frame_consts.params.vignette.color.z);
