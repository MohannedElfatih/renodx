...
struct ResourceViewSlots {
  ...
  bool Populate(reshade::api::command_list* cmd_list) {
    ...
    if (this->resources.empty()) return true;
  ...
  bool Render(
    ...
    if (this->auto_generate_descriptor_table_updates) {
      ...
      if (!this->shader_resource_slots.views.empty()) {
        ...
        current_vk_binding += is_vulkan ? srv_count : 0;
      }
      ...
      if (!samplers.empty()) {
        ...
        current_vk_binding += is_vulkan ? sampler_count : 0;
      }
      ...
      if (!this->unordered_access_slots.views.empty()) {
        ...
        current_vk_binding += is_vulkan ? uav_count : 0;
      }

#ifdef DEBUG_LEVEL_2
      // For Vulkan: verify binding indices are monotonically increasing within set 0.
      if (is_vulkan && !descriptor_table_updates.empty()) {
...
#ifdef DEBUG_LEVEL_2
          // Verify binding indices are monotonically increasing within the Vulkan set.
          uint32_t expected_binding = 0;
    ...
    if (is_graphics_pass && this->use_render_pass && render_pass_render_target_descs_size < render_target_views_size) {
      ...
      while (insert_index < render_target_views_size) {
        ...
        if (is_vulkan) {
          rt_desc.load_op = this->render_target_load_op;
          rt_desc.store_op = this->render_target_store_op;
        }
